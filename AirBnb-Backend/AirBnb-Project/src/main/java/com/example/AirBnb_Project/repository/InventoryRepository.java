...
public interface InventoryRepository extends JpaRepository<Inventory ,Long> {
    ...
    @Query("""
                ...
                WHERE i.room.id = :roomId
                  AND i.date BETWEEN :startDate AND :endDate
            """)
    @Lock(LockModeType.PESSIMISTIC_WRITE)        // while updating the inventory if someone come and alos try to book the room irt causes problem so better is to lock it for that particular time
    void getInventoryLockBeforeUpdate(@Param("roomId") Long roomId,
                                                 @Param("startDate") LocalDate startDate,
                                                 @Param("endDate") LocalDate endDate);

    List<Inventory> findByRoomIdAndDateBetween(Long id, LocalDate checkInDate, LocalDate checkOutDate);
}
